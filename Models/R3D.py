...
class R3DNet(nn.Module):
    ...
    def __init__(self, layer_sizes, block_type=SpatioTemporalResBlock):
        ...
        self.conv7 = SpatioTemporalResLayer(1024, 512, 3, layer_sizes[5], block_type=block_type, downsample=True)
...
class R3D(nn.Module):
    ...
    def __init__(self, layer_sizes=(2,2,2,2,2,2), block_type=SpatioTemporalResBlock, pretrained=False):
...
class R3D_For_VT(nn.Module):
    ...
    def __init__(self, layer_sizes=(2,2,2,2,2,2), block_type=SpatioTemporalResBlock, pretrained=False):
        super(R3D_For_VT, self).__init__()
...
class R3D_VT(nn.Module):
    def __init__(self, num_classes=2):
        super(R3D_VT, self).__init__()
        self.v_r3d = R3D_For_VT()#输出[batchsize,512]
        self.t_r3d = R3D_For_VT()

        self.fc = nn.Linear(1024, num_classes)
        self.relu = nn.ReLU()

    def forward(self, v_inputs, t_inputs, drop=None):
        v_out = self.v_r3d(v_inputs)
        t_out = self.t_r3d(t_inputs)#输出[batchsize,512]
