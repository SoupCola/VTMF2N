...
class UniformPatchEmbed(nn.Module):
    def __init__(self, img_size, patch_size, in_chans, embed_dim):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)#embed_dim为transformer嵌入的大小
...
class VIVIT(nn.Module):
    def __init__(self,img_size, patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio,qkv_bias,qk_scale, drop_rate, attn_drop_rate, drop_path_rate,num_frames,dropout, use_gpu=False):
        ...
        #Uniform sampling
        #simply split each frame into patches of size patch_size and then concatenate for all frames to 只需将每个帧拆分为patch_size大小的补丁，然后将所有帧连接到
        #form the batch
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        qkv = self.qkv(y).reshape(B, M, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # 让维度变为 B H N C
