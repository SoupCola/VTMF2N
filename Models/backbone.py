...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, modality, num_classes=1000, pool='avgpool', zero_init_residual=False,
        ...
        self.dropout4 = nn.Dropout(dropout_rate)  # 添加Dropout层
    ...
    def forward(self, x):
        ...
        if self.modality == 'visual' or self.modality == 'tactile':
            (B, T, C, H, W) = x.size()
        ...
        x = self.layer1(x)
        x = self.dropout1(x) #使用Dropout层
        x = self.layer2(x)
        x = self.dropout2(x) #使用Dropout层
        x = self.layer3(x)
        x = self.dropout3(x) #使用Dropout层
        x = self.layer4(x)
        x = self.dropout4(x) #使用Dropout层
        out = x
